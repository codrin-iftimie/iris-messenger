  ...
  Collection.prototype.get = function get() {
    ...
    var matcher = function matcher(data, id, node) {
      ...
      if (_this2.serializer) {
        opt.callback(
          _this2.serializer.deserialize(data, { id: id, gun: node.$ })
        );
...
var _isArrayIter = function (it) {
  return (
    it !== undefined &&
    (_iterators.Array === it || ArrayProto[ITERATOR$2] === it)
  );
};
...
try {
  isNode$2 =
    Object.prototype.toString.call(global$1.process) === "[object process]";
} catch (e) {}
...
async function loadGunDepth(chain) {
  ...
  return chain.then().then(function (layer) {
    ...
    if (
      maxDepth < 1 ||
      !layer ||
      (typeof layer === "undefined" ? "undefined" : _typeof(layer)) !==
        "object" ||
...
function arrayIndexOf$1(arr, val, byteOffset, encoding, dir) {
  ...
  if (dir) {
    ...
    for (i = byteOffset; i < arrLength; i++) {
      if (
        read$$1(arr, i) === read$$1(val, foundIndex === -1 ? 0 : i - foundIndex)
      ) {
...
var Attribute = (function () {
  ...
  Attribute.prototype.identiconXml = function identiconXml() {
    ...
    return util
      .getHash(
        encodeURIComponent(this.type) + ":" + encodeURIComponent(this.value),
        "hex"
      )
      .then(function (hash) {
...
var Contact = (function () {
  ...
  Contact.prototype.profileCard = function profileCard() {
    ...
    this.gun.on(async function (data) {
      ...
      var link =
        "https://iris.to/#/identities/" + linkTo.type + "/" + linkTo.value;
      var mva = Contact.getMostVerifiedAttributes(attrs);
      linkEl.innerHTML =
        '<a href="' +
        link +
      ...
      _Object$keys(attrs).forEach(function (k) {
        ...
        if (a.link) {
          links.innerHTML +=
            a.type + ': <a href="' + a.link + '">' + a.value + "</a> ";
        }
      });
    });
  ...
  Contact.prototype.identicon = function identicon$$1() {
    ...
    function setPie(data) {
        ...
        if (data.receivedPositive > data.receivedNegative) {
          transform =
            ...
            ((-data.receivedPositive /
              (data.receivedPositive + data.receivedNegative)) *
              360 -
              180) /
              2 +
            "deg)";
      ...
      pie.style.opacity =
        ((data.receivedPositive + data.receivedNegative) / 10) * 0.5 + 0.35;
    ...
    function setIdenticonImg(data) {
      util
        .getHash(
          encodeURIComponent(data.type) + ":" + encodeURIComponent(data.value),
          "hex"
        )
        .then(function (hash) {
...
var Channel = (function () {
  function Channel(options) {
    ...
    this.onTheir("participants", function (participants, k, from) {
      ...
      if (
        ...
        (_this.participants[from].admin || _this.participants[from].inviter)
      ) {
          ...
          _Object$keys(participants).forEach(function (k) {
            if (k !== _this.key.pub) {
              _this.addParticipant(
                k,
                true,
                _Object$assign({}, _this.DEFAULT_PERMISSIONS, participants[k])
              );
            }
          });
          options.saved = true;
      ...
      .on(async function (value, ourSecretChannelId) {
        if (value && !seen[ourSecretChannelId]) {
          ...
          var encryptedChatId = await util.gunOnceDefined(
            gun.user().get("chats").get(ourSecretChannelId).get("pub")
          );
  ...
  Channel.prototype.getLatestMsg = async function getLatestMsg(callback) {
    ...
    var callbackIfLatest = async function callbackIfLatest(msg, info) {
      if (!_this6.latest) {
        ...
        var t =
          typeof _this6.latest.time === "string"
            ? _this6.latest.time
            : _this6.latest.time.toISOString();
        if (t < msg.time) {
          _this6.latest = msg;
          callback(msg, info);
        }
  ...
  Channel.prototype.addParticipant = async function addParticipant(pub) {
    ...
    if (
      this.secrets[pub] &&
      _JSON$stringify(this.secrets[pub]) === _JSON$stringify(permissions)
    ) {
    ...
    if (typeof callback !== "function") {
      throw new Error(
        "onMy callback must be a function, got " +
          (typeof callback === "undefined" ? "undefined" : _typeof(callback))
      );
    }
    var keys = this.getCurrentParticipants();
      ...
      _this12.gun
        ...
        .on(async function (data) {
          var decrypted = await Gun.SEA.decrypt(
            data,
            await _this12.getSecret(keys[i])
          );
          if (decrypted) {
            callback(
    ...
    keys.forEach(async function (pub) {
      ...
      _this14.gun
        ...
        .on(async function (data) {
          var decrypted = await Gun.SEA.decrypt(
            data,
            await _this14.getTheirGroupSecret(pub)
          );
          if (decrypted) {
            callback(
              typeof decrypted.v !== "undefined" ? decrypted.v : decrypted,
  ...
  Channel.prototype.getChatLinks = async function getChatLinks(_ref) {
    ...
    this.on("chatLinks", function (links, from) {
      ...
      if (
        !links ||
        (typeof links === "undefined" ? "undefined" : _typeof(links)) !==
          "object"
      ) {
        return;
      }
  ...
  Channel.prototype.getChatBox = function getChatBox() {
    ...
    this.getMessages(function (msg, info) {
      ...
      if (info.selfAuthored) {
        var cls =
          _this18.theirMsgsLastSeenTime >= msg.time
            ? "iris-seen yes"
            : "iris-seen";
...
var SocialNetwork = (function () {
  ...
  SocialNetwork.prototype._init = async function _init() {
    ...
    if (this.options.self) {
      ...
      SignedMessage.createVerification({ recipient: recipient }, keypair).then(
        function (msg) {
        ...
        _this4.gun
          ...
          .once(function (val) {
            ...
            for (var i = 0; i < recipients.length; i++) {
              var a2 = recipients[i];
              if (
                (!Object.prototype.hasOwnProperty.call(o.attributes), a2.uri())
              ) {
  ...
  SocialNetwork.prototype.getRootContact = function getRootContact() {
    return new Contact(
      this.gun.get("identitiesBySearchKey").get(this.rootContact.uri()),
      undefined,
      this
    );
  };
        ...
        _Object$keys(attrs).forEach(function (k) {
          ...
          if (a.equals(attrs[k])) {
            ...
            attrs[k].unverifications =
              (attrs[k].unverifications || 0) + v.unverifications;
            hasAttr = true;
          }
        });
        if (!hasAttr) {
  ...
  SocialNetwork.prototype._updateContactIndexesByMsg = async function _updateContactIndexesByMsg(
    ...
    if (!_Object$keys(recipientIdentities).length) {
      ...
      var id = Contact.create(
        node,
        { attrs: attrs, linkTo: linkTo, trustDistance: trustDistance },
        this
      );
    ...
    var _loop2 = async function _loop2() {
      ...
      while (author && knownContact) {
        if (author.indexOf(knownContact.key) === 0) {
          try {
            await util.timeoutPromise(
              _this11.addMessage(msgsByAuthor[author], { checkIfExists: true }),
              10000
            );
          } catch (e) {
            _this11.debug(
    ...
    return new _Promise(function (resolve) {
      var resolveIfHashMatches = async function resolveIfHashMatches(d) {
        ...
        if (h === hash) {
          ...
          console.error(
            "queried index for message " + hash + " but received " + h
          );
        }
      };
      _this12.gun
          ...
          .once(function (val, key) {
            if (val) {
              _this12.gun
                ...
                .on(function (d) {
                  _this12.debug(
                    "got msg " + hash + " from friend's gun index " + val
                  );
                  resolveIfHashMatches(d);
                });
            }
...
var taggedTemplateLiteralLoose = createCommonjsModule(function (
  module,
  exports
) {
  exports.__esModule = true;
...
function k$1(l) {
  ((!l.__d && (l.__d = !0) && u.push(l) && !g$1.__r++) ||
    t !== n.debounceRendering) &&
    ((t = n.debounceRendering) || i$1)(g$1);
}
function g$1() {
  for (var n; (g$1.__r = u.length); )
...
function m(n, l, u, i, t, o, r, c, s, h) {
  var y,
    ...
    P = A.length;
  for (
    s == f$8 && (s = null != r ? r[0] : P ? _(i, 0) : null), u.__k = [], y = 0;
    y < l.length;
    y++
  )
    if (
...
function H(n, l, u, i, t, o, r, c) {
  ...
  if (null === l.type) p === d || (c && n.data === d) || (n.data = d);
    ...
    if (
      ...
      (h = (p = u.props || f$8).dangerouslySetInnerHTML),
...
(d.prototype.setState = function (n, l) {
  ...
  n && s(u, n),
  null != n && this.__v && (l && this.__h.push(l), k$1(this));
}),
  ...
  return (r$1 =
    ...
    function (t) {
      for (var e = 1; e < arguments.length; e++) {
        var n$$1 = arguments[e];
        for (var o in n$$1)
          Object.prototype.hasOwnProperty.call(n$$1, o) && (t[o] = n$$1[o]);
      }
      return t;
  ...
  var e = t.children,
    n$$1 = (function (t, e) {
      ...
      for (o = 0; o < i.length; o++)
        e.indexOf((n$$1 = i[o])) >= 0 || (r[n$$1] = t[n$$1]);
      return r;
    })(t, ["context", "children"]);
  return q(e, n$$1);
}
function a$1() {
  ...
  (this._vdom = h(
    i$2,
    r$1({}, this._props, { context: o.detail.context }),
    (function e(n$$1, o) {
      if (3 === n$$1.nodeType) return n$$1.data;
      if (1 !== n$$1.nodeType) return null;
      var r = [],
        i = {},
        a = 0,
        c = n$$1.attributes,
      ...
      for (a = c.length; a--; )
        "slot" !== c[a].name &&
            ((i[c[a].name] = c[a].value), (i[s$1(c[a].name)] = c[a].value));
      for (a = l$$1.length; a--; ) {
        var p$$1 = e(l$$1[a], null),
          d$$1 = l$$1[a].slot;
        d$$1 ? (i[d$$1] = h(u$1, { name: d$$1 }, p$$1)) : (r[a] = p$$1);
      }
      var h$$1 = o ? h(u$1, null, r) : r;
      return h(o || n$$1.nodeName.toLowerCase(), i, h$$1);
        ...
        t
          ...
          o._listener ||
              ((o._listener = function (t) {
                t.stopPropagation(), (t.detail.context = n$$1);
              }),
...
function register(t, e, n$$1, o) {
  ...
  return (
    ...
    (r.prototype.connectedCallback = a$1),
    (r.prototype.attributeChangedCallback = c$1),
    (r.prototype.disconnectedCallback = l$1),
    (n$$1 = n$$1 || t.observedAttributes || Object.keys(t.propTypes || {})),
    (r.observedAttributes = n$$1),
...
var extendStatics = function (d$$1, b$$1) {
  extendStatics =
    ...
    function (d$$1, b$$1) {
      for (var p$$1 in b$$1)
        if (b$$1.hasOwnProperty(p$$1)) d$$1[p$$1] = b$$1[p$$1];
    };
...
function __extends(d$$1, b$$1) {
  ...
  d$$1.prototype =
    b$$1 === null
      ? Object.create(b$$1)
      : ((__.prototype = b$$1.prototype), new __());
}
...
var Identicon = (function (_Component) {
  ...
  Identicon.prototype.componentDidUpdate = function componentDidUpdate(
    prevProps
  ) {
  ...
  Identicon.prototype.componentDidMount = function componentDidMount() {
    ...
    new Attribute$1({ type: "keyID", value: this.props.user })
      .identiconSrc({ width: this.props.width, showType: false })
      .then(function (identicon) {
  ...
  Identicon.prototype.render = function render() {
    return m$1(
      ...
      parseInt(this.props.width) || DEFAULT_WIDTH,
      this.props.showTooltip ? "tooltip" : "",
      this.props.showTooltip && this.state.name
        ? m$1(_templateObject2, this.state.name)
        : "",
...
!util.isNode &&
  register(Identicon, "iris-identicon", [
    "user",
    "onClick",
    "width",
    "showTooltip",
...
var TextNode = (function (_Component) {
  ...
  TextNode.prototype.componentDidUpdate = function componentDidUpdate(
    prevProps
  ) {
    if (
  ...
  TextNode.prototype.getValue = function getValue(user) {
    ...
    this.getNode(user).on(function (value, a, b$$1, e) {
      ...
      if (
        !(_this3.ref.current && _this3.ref.current === document.activeElement)
      ) {
        _this3.setState({
          value: value,
  ...
  TextNode.prototype.isEditable = function isEditable() {
    return (
      (!this.props.user || this.props.user === this.state.myPub) &&
      String(this.props.editable) !== "false"
    );
  };
  ...
  TextNode.prototype.renderTag = function renderTag() {
    ...
    return m$1(
      ...
      this.props.json ? _JSON$stringify(this.state.value) : this.state.value,
      tag
    );
  };
...
var ImageNode = (function (_TextNode) {
  ...
  ImageNode.prototype.render = function render() {
    ...
    var src =
      val && val.indexOf("data:image") === 0 ? val : this.props.placeholder;
...
var CopyButton = (function (_Component) {
  ...
  CopyButton.prototype.onClick = function onClick(e) {
    ...
    e.preventDefault();
    var str =
      typeof this.props.str === "function" ? this.props.str() : this.props.str;

    if (navigator.share && util.isMobile && !this.props["not-shareable"]) {
      navigator
        .share({ url: str, title: this.props.title })
        .catch(function (err) {
...
!util.isNode &&
  register(CopyButton, "iris-copy-button", [
    "str",
    "not-shareable",
    "text",
    "copied-text",
  ...
  FollowButton.prototype.componentDidMount = function componentDidMount() {
    ...
    Key$1.getDefault().then(function (key) {
      ...
      util
        ...
        .user()
        .get("follow")
        .get(_this2.props.user)
        .on(function (following, a, b$$1, e) {
  ...
  if (
    ...
    C = wrapper(function (target, iterable) {
      ...
      if (iterable != undefined)
        _forOf(iterable, IS_MAP, target[ADDER], target);
    });
    each(
      ...
      function (KEY) {
        ...
        if (KEY in proto && !(IS_WEAK && KEY == "clear"))
          _hide(C.prototype, KEY, function (a, b) {
            ...
            if (!IS_ADDER && IS_WEAK && !_isObject(a))
...
var Search = (function (_Component) {
  ...
  Search.prototype.getFollowsFn = function getFollowsFn(callback, k) {
    ...
    util
      ...
      .get("follow")
      .map()
      .once(function (isFollowing, followedKey) {
    ...
    return m$1(
      ...
      this.state.results.map(function (r) {
        ...
        if (i.followDistance === 2) {
          if (
            i.followers.size === 1 &&
            _this4.follows[[].concat(i.followers)[0]] &&
            _this4.follows[[].concat(i.followers)[0]].name
          ) {
            followText =
              "Followed by " + _this4.follows[[].concat(i.followers)[0]].name;
          } else {
            followText =
              "Followed by " + i.followers.size + " users you follow";
          }
        }
        return m$1(
          _templateObject2$3,
